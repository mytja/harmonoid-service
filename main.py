...
def Hello():
    return Response(
        "service is running", status=200, headers={"Content-Type": "text/html"}
    )
...
@main.route("/search")
def SearchSpotify():
    return harmonoidService.SearchSpotify(
        request.args.get("keyword", None),
        request.args.get("mode", "album"),
...
def SearchYoutube():
    return harmonoidService.SearchYoutube(
        request.args.get("keyword", None),
        int(request.args.get("offset", 1)),
        request.args.get("mode", "json"),
        int(request.args.get("max_results", 10)),
    )
...
def TrackDownload():
    return harmonoidService.TrackDownload(
        request.args.get("track_id", None), request.args.get("track_name", None)
    )
