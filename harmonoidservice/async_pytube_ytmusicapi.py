...
class YouTube(YouTube):
    ...
    async def getJS(self) -> None:
        ...
        if pytube.__js_url__ != self.js_url:
            async with httpx.AsyncClient() as client:
                response = await client.get(self.js_url)
    ...
    async def decipher(self, retry: bool = False):
        ...
        try:
            ...
            Used to decipher the stream URLs using player JavaScript & the player_response passed from the constructor of this derieved class.
            ...
            apply_signature(self.player_response, "url_encoded_fmt_stream_map", pytube.__js__)
        except:
            """
...
class YTMusic(YTMusic):
    ...
    async def getSong(self, videoId: str) -> Dict:
        ...
        This method makes zero network requests & multiple requests being made by using vanilla PyTube & YTMusic. (*cough. Considereing player JavaScript doesn't update out of nowhere)
        """
